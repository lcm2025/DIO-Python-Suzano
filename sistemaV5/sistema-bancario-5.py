...
class Conta:
    ...
    def exibir_extrato(self, conexao, cursor):
        cursor.execute("SELECT operacao, valor, datetime(data, 'localtime') FROM historico WHERE num_conta=?", (self.num_conta,))
        historico_conta = cursor.fetchall()
        if not historico_conta:
            print("\n Nenhuma operação registrada para essa conta.")
            return
        
        print("\n=== Extrato da Conta ===")
        for operacao, valor, data in historico_conta:
            print(f"{data} - {operacao} - {valor: .2f}")
            print("========================")
...
class Banco:
    ...
    def extrato(self):
        ...
        if not self.buscar_cpf(cpf_cliente):
            print("Cliente não encontrado. Verifique o CPF e tente novamente.")
            return
        cursor.execute("SELECT num_conta, saldo FROM conta WHERE cpf=?", (cpf_cliente,))
        conta_info = cursor.fetchone()
        if not conta_info:
            print("Nenhuma conta encontrada para este CPF.")
            return
        num_conta, saldo = conta_info
        conta = Conta(num_conta, cpf_cliente, saldo)
        conta.exibir_extrato(conexao, cursor)
